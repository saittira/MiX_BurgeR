...
const productsData = [
    ...
    { id: 'sn15', category: 'snacks', name_ru: 'Сyrные палочки (5 шт)', name_kg: 'Сыр таякчалары (5 даана)', price: 200 },
...
const translations = {
    ...
    kg: {
        ...
        heroDesc: "Ширелүү бургерлер, өзгөчө шаурма... Онлайн буйрутма бериңиз — өзүңүз алып кетиңиз же биз жеткиребиз.",
...
const whatsappPhoneNumber = "996504400448"; 
...
function toggleLanguage() {
    ...
    const qrPaymentTitle = document.getElementById('lang-qr-payment-title');
    if(qrPaymentTitle) qrPaymentTitle.innerText = t.qrTitle;
    
    const qrInstruction = document.getElementById('lang-qr-instruction');
    if(qrInstruction) qrInstruction.innerText = t.qrInstruction;
    
    const lblReceipt = document.getElementById('lang-lbl-receipt');
    if(lblReceipt) lblReceipt.innerText = t.lblReceipt;
    
    const uploadBtn = document.getElementById('lang-upload-btn');
    if(uploadBtn) uploadBtn.innerText = t.uploadBtn;
    
    const waNotice = document.getElementById('lang-wa-notice');
    if(waNotice) waNotice.innerText = t.waNotice;
...
function updateCartUI() {
    ...
    const qrTotalDisplay = document.getElementById('qr-total-display');
    if(qrTotalDisplay) qrTotalDisplay.innerText = totalPrice + ' С';
...
function handleFileSelect(input) {
    if (input.files && input.files[0]) {
        ...
        if(box) box.innerHTML = `🟢 <span style="color:#fff; font-weight:bold;">${input.files[0].name}</span>`;
    }
}
